...
##基于AJCC_stage分2层 stage1-stage2/stage3-stage4
##对临床字段进行处理
...
AJCC.OS.survival.data <- lapply(strata.OS.byDriverGene.ClonalSubclonal,function(x){
	                                split(x,x$TNM_stage)
});
names(AJCC.OS.survival.data) <- names(OS.byDriverGene.ClonalSubclonal)
...
AJCC_medianOS_survivalRate <- list()
for(i in names(AJCC.OS.survival.data)){
  print(i);
	for(j in 1:length(AJCC.OS.survival.data[[i]])){
	  inputFilePath <- paste0(file.path("F:/Rsources/Project/StrataByAJCC",i,"AJCC_stage",unique(AJCC.OS.survival.data[[i]][[j]]$TNM_stage)),"/");
		#要有三类样本
		if(length(table(AJCC.OS.survival.data[[i]][[j]]$sample.label)) >= 2 & min(table(AJCC.OS.survival.data[[i]][[j]]$sample.label)) >= 5){		    
		  AJCC_medianOS_survivalRate[[i]] <- plot.surv(clinical.data = AJCC.OS.survival.data[[i]][[j]],upper.time=NULL, unit.xlabel = "month",
...
AJCC.DFS.survival.data <- lapply(strata.DFS.byDriverGene.ClonalSubclonal,function(x){
	   split(x,x$TNM_stage);
})
names(AJCC.DFS.survival.data) <- names(DFS.byDriverGene.ClonalSubclonal)
...
#KM生存曲线如果有1组样本数太少会报错，一般至少为5
AJCC_medianDFS_survivalRate <- list()
for(i in names(AJCC.DFS.survival.data)){
  print(i);
	for(j in 1:length(AJCC.DFS.survival.data[[i]])){
	  inputFilePath <- paste0(file.path("F:/Rsources/Project/StrataByAJCC",i,"AJCC_stage",unique(AJCC.DFS.survival.data[[i]][[j]]$TNM_stage)),"/")
		#要有两类样本并且每类样本个数都要大于5
		if(length(table(AJCC.DFS.survival.data[[i]][[j]]$sample.label)) >= 3 & min(table(AJCC.DFS.survival.data[[i]][[j]]$sample.label)) > 5){		    
		   AJCC_medianDFS_survivalRate[[i]] <- plot.surv(clinical.data = AJCC.DFS.survival.data[[i]][[j]],upper.time=NULL, unit.xlabel = "month",
                                                     risk.table = TRUE, pval = TRUE, conf.int = F, surv.median.line="hv", 
