...
MSS.OS.survival.data <- lapply(strata.OS.byDriverGene.ClonalSubclonal,function(x){
    split(x,x$MSS_state)
})
names(MSS.OS.survival.data) <- names(OS.byDriverGene.ClonalSubclonal);
...
MSS_medianOS_survivalRate <- list()
for(i in names(MSS.OS.survival.data)){
  print(i);
  for(j in 1:length(MSS.OS.survival.data[[i]])){
    inputFilePath <- paste0(file.path("F:/Rsources/Project/StrataByMSS",i,"MSS_state",unique(MSS.OS.survival.data[[i]][[j]]$MSS_state)),"/");
    #要有三类样本
    if(length(table(MSS.OS.survival.data[[i]][[j]]$sample.label)) >= 2 & min(table(MSS.OS.survival.data[[i]][[j]]$sample.label)) >= 5){		    
       MSS_medianOS_survivalRate[[i]] <- plot.surv(clinical.data = MSS.OS.survival.data[[i]][[j]],upper.time=NULL, unit.xlabel = "month",
...
MSS.DFS.survival.data <- lapply(strata.DFS.byDriverGene.ClonalSubclonal,function(x){
         split(x,x$MSS_state)
})
names(MSS.DFS.survival.data) <- names(DFS.byDriverGene.ClonalSubclonal)
...
#KM生存曲线如果有1组样本数太少会报错，一般至少为5
MSS_medianDFS_survivalRate <- list()
for(i in names(MSS.DFS.survival.data)){
  print(i);
  for(j in 1:length(MSS.DFS.survival.data[[i]])){
    inputFilePath <- paste0(file.path("F:/Rsources/Project/StrataByMSS",i,"MSS_state",unique(MSS.DFS.survival.data[[i]][[j]]$MSS_state)),"/");
    #要有两类样本并且每类样本个数都要大于5
    if(length(table(MSS.DFS.survival.data[[i]][[j]]$sample.label)) >= 2 & min(table(MSS.DFS.survival.data[[i]][[j]]$sample.label)) > 5){		    
      MSS_medianDFS_survivalRate[[i]] <- plot.surv(clinical.data = MSS.DFS.survival.data[[i]][[j]],upper.time=NULL, unit.xlabel = "month",
                                                    risk.table = TRUE, pval = TRUE, conf.int = F, surv.median.line="hv", 
